...
func Logger(logger *zap.Logger, opts ...Option) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		if cfg.enableBody && c.Request.Body != nil {
		...
		defer func() {
			...
			if len(c.Errors) > 0 {
				...
				if respStatusCode >= http.StatusInternalServerError {
					logger.Error("req logged", fields...)
				} else if respStatusCode >= http.StatusBadRequest {
					logger.Warn("req logged", fields...)
				} else {
					logger.Debug("req logged", fields...)
				}
			}
		}()
